...
<div class="action divaction">
    ...
    <div class="pages">
        <!--员工信息区域 开始-->
        <form action="<%=request.getContextPath() %>/control/employee?opr=modify&id=${employee.id}"
              enctype="multipart/form-data" name="form1" method="post" onsubmit="return check()">
            <table class="leave">
                <tr>
                    <td class="width1"><label>姓名：</label></td>
                    <td class="width2"><input type="text" id="name" name="name" value="${employee.name}"/></td>
                    <td class="width1"><label>工号：</label></td>
                    <td class="width2"><input type="text" id="employeeNo" name="employeeNo"
                                              value="${employee.employeeNo}" readonly/></td>
                </tr>
                <tr>
                    <td class="width1"><label>email：</label></td>
                    <td class="width2"><input type="text" id="email" name="email" value="${employee.email}"/></td>
                    <td class="width1"><label>入职时间：</label></td>
                    <td class="width2"><input class="Wdate" onFocus="WdatePicker({lang:'zh-cn'})" name="entryTime"
                                              value="${employee.entryTime}"/></td>
                </tr>
                <tr>
                    ...
                    <td class="width2"><select name="department" theme="simple" cssClass="nwselect">
                        <c:forEach var="department" items="${sessionScope.departmentList}">
                            <c:choose>
                                <c:when test="${employee.deptId eq department.id}">
                    ...
                    <td class="width2"><select name="position" theme="simple" cssClass="nwselect">
                        <c:forEach var="position" items="${sessionScope.positionList}">
                            <c:choose>
                                <c:when test="${employee.positionId eq position.id}">
                    ...
                    <td colspan="3">
                        <c:choose>
                            <c:when test="${employee.photo ne null}">
                                <img src="<%=request.getContextPath() %>/${employee.photo}" width="96" height="96"
