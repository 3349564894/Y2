...
var passtmp;//密码缓存
var verifytmp;//验证码缓存
var userTeltip;//用户电话提示框
var telExist = true; //设置电话是否已被注册
/* 初始化上面的这些对象变量 */
function init() {
    ...
    userTeltip = document.getElementById("userTeltip");
    userIDtip = document.getElementById("userIDtip");
    userNametip = document.getElementById("userNametip");
    userMailtip = document.getElementById("userMailtip");
    userPasstip = document.getElementById("userPasstip");
    userRpasstip = document.getElementById("userRpasstip");
}

/* 检测真实姓名输入框 */
function checkRealName(imgobj, textobj) {
    userIDtip.style.visibility = "hidden";
...
/* 对象获得对象时的操作 */
function getfocus(textobj, imgobj, tipObj) {
    //alert(tipObj);

    if ($(tipObj).html() == "手机号已被注册！") {
        $(tipObj).html("请输入您的手机号码，之后作为登录账号");
    }
    imgobj.style.visibility = "hidden";//设置相应的图像对象隐藏
    textobj.style.backgroundColor = "#fff";//设置相应的输入框背景色为白色
    tipObj.style.visibility = "visible"; //设置提示对象隐藏
}

/* 检测用户昵称的长度大于等于4,则通过检测 */
function checkNickName(imgobj, textobj) {
    userNametip.style.visibility = "hidden";
...
function checkUserTel(imgobj, textobj) {
    ...
    if (textobj.value.match(/\d+/g) != null && textobj.value.match(/\d+/g)[0].length == 11) {
        checkUserTelExist(imgobj, textobj);
        // textobj.style.backgroundColor = "#fff";
        // imgobj.src = "../images/ok.png";
        return true;
    }
...
function checkUserMail(imgobj, textobj) {
    userMailtip.style.visibility = "hidden";
...
function checkUserPass(imgobj, textobj) {
    userPasstip.style.visibility = "hidden";
...
function checkUserRpass(imgobj, textobj) {//检测重复的密码
    userRpasstip.style.visibility = "hidden";
    imgobj.style.visibility = "visible";
    textobj.value = trim(textobj.value);
...
function checkForm() {
    if (checkRealName(img1, realName) == false) return false;
    if (checkNickName(img2, nickName) == false) return false;
    ...
    if (telExist) return false;
    if (checkSMSCode(verify) == false) return false;
